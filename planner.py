...
def generate_plan(goal: str, user: str) -> List[Dict[str, object]]:
    ...
    if "search" in g or "look up" in g:
        ...
        plan.append({"agent": "web_search", "action": "search_web", "params": {"query": query, "user": user}})

    # Downloading files
    url_match = re.search(r"https?://\S+", goal)
    if "download" in g and url_match:
        url = url_match.group(0)
        plan.append({"agent": "file_downloader", "action": "download_file", "params": {"url": url, "user": user}})
    ...
    # Browser automation
    url_browse = re.search(r"https?://\S+", goal)
    if ("go to" in g or "open" in g or "fill" in g) and url_browse:
        steps = [{"action": "go_to", "url": url_browse.group(0)}]
        plan.append({
            "agent": "browser_agent",
